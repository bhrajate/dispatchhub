...
> 这是一份**可复用流程**，把 [../performance/optimizations/r1-postmortem-2026-05-19-write-path.md](../performance/optimizations/r1-postmortem-2026-05-19-write-path.md)
> 那次"vegeta→wrk2→profile→GORM 默认事务"过程里抽象出来的步骤、工具、判断准则
> 沉淀下来。下次再遇到"某接口饱和 / 某请求慢 / 某 CPU 跑满"，按这个流程走就行，
> 不要从零靠经验拍。
>
> 适用场景：
...
> **看代码猜瓶颈基本不靠谱，profile 才靠谱**。模式匹配（"我见过这种慢"）
> 给出的假设质量低，因为同一段代码常常同时撞了 3-5 个常见模式，全靠 profile
> 排序。**先量化、再下结论**。
...
**目标**：拿到一份可对比的 baseline。**不要在没有 baseline 的情况下做"优化"**——
不然修完了不知道是真有效还是迷信。
...
原因：closed-model 的 RPS 受服务端延迟反向调制，"看起来稳"但其实是被服务端拖
住，看不出饱和点。本仓库 wrk2 driver 用 `-R <rps>` 强制注入，ghz 用 `--rps`。
...
这个阶梯就是 profile 要打的目标 RPS。后面所有 phase 都围绕**这个 RPS 值**做。

⚠️ 不要在未饱和阶梯抓 profile：CPU 没跑满，热点不会显出来，你看到的是空载下
的相对噪声。
...
本次案例里：读路径 1000 RPS = CPU 164%（多核并行），写路径 300 RPS = CPU 105%
（单核满载）—— 同一进程不同接口，CPU 模式不同，本身就是"写路径有串行点"的
强证据。
...
如果你的服务**还没**挂 pprof，照这两个文件抄。这是一次性投入，下一次性能调查
直接用。
...
⚠️ **一定要在饱和 RPS 下抓**，不饱和的 profile 没有意义（参 §3）。
...
`-cum`（累计）比 `flat`（自身）有用得多——上层 wrapper 的 flat 通常是 0，但 cum
告诉你"经过它的请求总共花了多少时间"。
...
| futex / scheduler | `runtime.futex` + `runtime.findRunnable` 高 | 锁争用，看 mutex profile |

### 当 CPU profile 看不出问题时

CPU profile 只看"在 CPU 上跑的时间"。如果瓶颈是**等锁、等 IO、等 channel**，
goroutine 不在 CPU 上，cpu profile 看不到。换工具：
...
# Block (channel send/recv 阻塞)
curl http://127.0.0.1:8080/debug/pprof/block -o block.prof

# 完整 trace（30s 内所有 goroutine 状态）
curl 'http://127.0.0.1:8080/debug/pprof/trace?seconds=30' -o trace.out
go tool trace trace.out
```

mutex 与 block profile 默认采样率为 0，需要在代码里 `runtime.SetMutexProfileFraction(1)`
+ `runtime.SetBlockProfileRate(1)` 启用（仅 benchmark build 开）。
...
**检查方法**：每次跑完压测确认 `cell_dir/pidstat.txt` 有内容**且**包含被测进程
PID 行。
...
- 多个改动可能互相掩盖：A 让锁不再是瓶颈，B（缓存）就显得没用——其实 B 单独
  上也有用
- 出问题时 bisect 难度爆炸
...
只看一个维度容易自欺。本次案例里写路径 RPS +51% / p99 −73% / CPU −43% 互相
印证，置信度高；如果只 RPS +51% 而 errors 同步 +50%，那是把错误吞掉而不是
真优化。
