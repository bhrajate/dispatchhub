...
### 3.6 核心难点五：一致性哈希与最小化任务重分配

当 Worker 节点动态增减时（比如 HPA 扩缩容），我需要一种机制来保证任务分配的稳定性。我用了**一致性哈希环 + 150 个虚拟节点**的方案。

为什么是 150 个虚拟节点？虚拟节点太少，哈希环上的分布不均匀，会出现某些 Worker 分到的任务远多于其他的；太多则占用更多内存和查找时间。150 是经过测试后在均衡性和性能之间的一个好的平衡点。

关键好处是：当一个 Worker 下线时，只有映射到它的那部分任务需要重新分配到相邻节点，其他 Worker 的任务分配完全不变。这比简单的取模哈希（`taskID % workerCount`）好得多——取模方案在 Worker 数量变化时会导致几乎所有任务重新分配。
...
Asynq 确实和 DispatchHub 的思路类似，都是基于 Redis 的任务队列。但 Asynq 缺少控制面/数据面分离的架构、没有一致性哈希负载均衡、缺少 etcd 级别的 Leader 选举。Temporal 更侧重工作流编排，对于简单的任务调度场景太重了。自研的好处是完全贴合业务需求，也让团队对核心调度逻辑有完整的掌控力。
