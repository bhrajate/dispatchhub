package types

import "time"

// WorkerState represents the health state of a worker node.
type WorkerState int

const (
	WorkerStateOnline  WorkerState = iota
	WorkerStateDraining            // accepting no new tasks, finishing current
	WorkerStateOffline
)

var workerStateNames = map[WorkerState]string{
	WorkerStateOnline:   "online",
	WorkerStateDraining: "draining",
	WorkerStateOffline:  "offline",
}

func (s WorkerState) String() string {
	if name, ok := workerStateNames[s]; ok {
		return name
	}
	return "unknown"
}

// WorkerInfo describes a registered worker node in the cluster.
type WorkerInfo struct {
	ID            string            `json:"id"`
	Hostname      string            `json:"hostname"`
	IP            string            `json:"ip"`
	Port          int               `json:"port"`
	State         WorkerState       `json:"state"`
	Labels        map[string]string `json:"labels,omitempty"`        // node selectors (e.g., "gpu": "true")
	Queues        []string          `json:"queues"`                  // queues this worker subscribes to
	Concurrency   int               `json:"concurrency"`             // max concurrent tasks
	ActiveTasks   int               `json:"active_tasks"`
	CompletedTotal int64            `json:"completed_total"`
	FailedTotal   int64             `json:"failed_total"`
	CPUUsage      float64           `json:"cpu_usage"`
	MemUsage      float64           `json:"mem_usage"`
	StartedAt     time.Time         `json:"started_at"`
	LastHeartbeat time.Time         `json:"last_heartbeat"`
	Version       string            `json:"version"`
}

// IsAvailable returns true if the worker can accept new tasks.
func (w *WorkerInfo) IsAvailable() bool {
	return w.State == WorkerStateOnline && w.ActiveTasks < w.Concurrency
}

// Load returns the current load ratio (0.0 to 1.0).
func (w *WorkerInfo) Load() float64 {
	if w.Concurrency == 0 {
		return 1.0
	}
	return float64(w.ActiveTasks) / float64(w.Concurrency)
}

// Heartbeat carries periodic worker status to the scheduler.
type Heartbeat struct {
	WorkerID    string    `json:"worker_id"`
	State       WorkerState `json:"state"`
	ActiveTasks int       `json:"active_tasks"`
	CPUUsage    float64   `json:"cpu_usage"`
	MemUsage    float64   `json:"mem_usage"`
	Timestamp   time.Time `json:"timestamp"`
}
